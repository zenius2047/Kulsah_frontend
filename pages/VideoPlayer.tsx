...
const normalizeFeedVideo = (item?: FeedRouteItem | null): VideoItem | null => {
  ...
  return {
    ...
    duration: '',
    img: item.background ?? item.avatar ?? '',
    url: item.video,
    date: '',
...
const normalizeCreatorVideo = (item?: CreatorVideoPreview | null): VideoItem | null => {
  ...
  return {
    ...
    img: item.background ?? item.img ?? item.thumbnail ?? item.thumbnail_url ?? item.avatar ?? '',
    url: item.video ?? '',
    date: item.date ?? '',
...
/** Keeps only the feed-sized window around the active item prepared. */
const VideoPreloader = React.memo(({ source }: { source: string }) => {
  const preloadPlayer = useVideoPlayer(source, (instance) => {
    instance.loop = false;
    instance.muted = true;
...
const VideoPlayer: React.FC = () => {
  ...
  const currentVerticalIndex = Math.max(0, verticalVideos.findIndex((item) => item.id === video.id));
  const nearbyVideos = verticalVideos
    .slice(
      Math.max(0, currentVerticalIndex - PRELOAD_BEHIND),
      Math.min(verticalVideos.length, currentVerticalIndex + PRELOAD_AHEAD + 1),
    )
    .filter((item) => item.id !== video.id && Boolean(item.url));
    ...
    <SafeAreaView style={styles.shortPlayerRoot} edges={[]}>
      ...
      >
        {stageHeight > 0 ? <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.shortVideoPage, { height: stageHeight }]}>
              <VideoFeedItem
                ...
                onSubscribe={() => setToast('Subscriptions')}
                onFollow={() => setFollowed((value) => !value)}
                onToggleLike={(_, nextLiked) => setLiked(nextLiked)}
                onToggleBookmark={() => saveToLibrary()}
                onRecordView={() => undefined}
                isGlobalMuted={muted}
          ...
          getItemLayout={(_, index) => ({ length: stageHeight, offset: stageHeight * index, index })}
...
const styles = StyleSheet.create({
  ...
  shortOverflowText: { flex: 1, color: '#fff', fontSize: 15, fontWeight: '500' },
  shortOverflowValue: { color: '#bdbdbd', fontSize: 14, fontWeight: '600' },
  ...
  railCount: { color: '#fff', fontSize: 16, lineHeight: 21, fontWeight: '600', textShadowColor: 'rgba(0,0,0,0.8)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  ...
  shortTitle: { color: '#fff', fontSize: 20, lineHeight: 25, fontWeight: '700', textShadowColor: 'rgba(0,0,0,0.85)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4 },
  shortDescription: { color: '#f1f1f1', fontSize: 17, lineHeight: 22, fontWeight: '400', textShadowColor: 'rgba(0,0,0,0.85)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4 },
  ...
  episodeText: { color: '#d7d7d7', fontSize: 17, fontWeight: '500' },
  ...
  speedText: { color: '#d7d7d7', fontSize: 17, fontWeight: '500' },
  ...
  header: { minHeight: 56, paddingHorizontal: 12, paddingBottom: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, headerBrand: { flexDirection: 'row', alignItems: 'center', gap: 8 }, headerTitle: { ...fontSize.b3, lineHeight: fontSize.b3.fontSize + 2 }, headerIconBtn: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' }, headerActions: { flexDirection: 'row', alignItems: 'center', gap: 10 }, iconBtn: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center', borderWidth: 1 }, pill: { flexDirection: 'row', alignItems: 'center', gap: 8, height: 42, paddingHorizontal: 14, borderRadius: 999, borderWidth: 1 }, pillSmall: { height: 36, minWidth: 58, borderRadius: 18, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 }, pillText: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 },
  videoTopOverlay: { position: 'absolute', left: 12, right: 12, top: 0, zIndex: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, videoOverlayButton: { width: 42, height: 42, borderRadius: 21, backgroundColor: 'rgba(0,0,0,0.28)', alignItems: 'center', justifyContent: 'center' }, videoOverlayPill: { height: 34, minWidth: 54, borderRadius: 17, paddingHorizontal: 12, backgroundColor: 'rgba(0,0,0,0.34)', alignItems: 'center', justifyContent: 'center' }, videoOverlayText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 },
  videoShell: { overflow: 'hidden', borderWidth: 0, backgroundColor: '#000', height: 250}, video: { ...StyleSheet.absoluteFillObject }, videoLoaderOverlay: { ...StyleSheet.absoluteFillObject, alignItems: 'center', justifyContent: 'center', backgroundColor: '#000', zIndex: 4 }, heatmap: { position: 'absolute', bottom: 60, left: 16, right: 16, height: 52, flexDirection: 'row', alignItems: 'flex-end', gap: 2 }, heatBar: { flex: 1, borderTopLeftRadius: 4, borderTopRightRadius: 4 }, controls: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center' }, controlsRow: { flexDirection: 'row', alignItems: 'center', gap: 20 }, ghostBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(0,0,0,0.48)', alignItems: 'center', justifyContent: 'center' }, playBtn: { width: 70, height: 70, borderRadius: 35, backgroundColor: 'rgba(0,0,0,0.58)', alignItems: 'center', justifyContent: 'center' }, muteBtn: { position: 'absolute', right: 14, top: 14, width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(0,0,0,0.55)', alignItems: 'center', justifyContent: 'center' }, fullscreenBtn: { position: 'absolute', right: 14, bottom: 58, width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(0,0,0,0.55)', alignItems: 'center', justifyContent: 'center' }, orientationToggleBtn: { position: 'absolute', right: 56, bottom: 58, height: 36, borderRadius: 18, paddingHorizontal: 10, backgroundColor: 'rgba(0,0,0,0.55)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }, orientationToggleText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, landscapeBadge: { position: 'absolute', left: 14, top: 14, height: 28, borderRadius: 14, paddingHorizontal: 10, backgroundColor: 'rgba(0,0,0,0.55)', flexDirection: 'row', alignItems: 'center', gap: 5 }, landscapeBadgeText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, videoFooter: { position: 'absolute', left: 12, right: 12, bottom: 10 }, track: { height: 3, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.34)', overflow: 'hidden' }, fill: { height: '100%', borderRadius: 999, backgroundColor: PRIMARY_COLOR }, metaRow: { marginTop: 8, flexDirection: 'row', justifyContent: 'space-between' }, metaText: { color: 'rgba(255,255,255,0.88)', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 },
  ...
  titleBlock: { gap: 8 }, title: { color: '#fff', ...fontSize.b3, lineHeight: fontSize.b3.fontSize + 5 }, titleHeart: { color: '#ff3b6b' }, descriptionRow: { width: '95%', flexDirection: 'row', gap: 5 }, descriptionCopy: { flex: 1 }, descriptionText: { marginTop: 2, ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 4 }, titleMeta: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', columnGap: 8, rowGap: 3 }, smallMeta: { color: '#aaa', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, moreText: { color: '#fff', marginTop: 2, ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 4 }, dot: { width: 3, height: 3, borderRadius: 1.5 }, actionRow: { gap: 8, paddingRight: 12 }, actionChip: { flexDirection: 'row', alignItems: 'center', gap: 7, height: 36, paddingHorizontal: 14, borderRadius: 18 }, actionText: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 },
  creatorActionRow: { flexDirection: 'row', alignItems: 'center', gap: 14, paddingTop: 2, paddingBottom: 2, zIndex: 20 }, creatorMini: { width: 38, height: 38, borderRadius: 19, overflow: 'hidden' }, iconAction: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center' }, subscribeButton: { height: 38, borderRadius: 19, paddingHorizontal: 18, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' }, subscribeText: { color: '#111', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, artistMain: { flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }, artistAvatar: { width: 38, height: 38, borderRadius: 19 }, artistRow: { flexDirection: 'row', alignItems: 'center', gap: 5 }, artistName: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, artistHandle: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, followButton: { height: 36, borderRadius: 18, paddingHorizontal: 16, alignItems: 'center', justifyContent: 'center' }, followText: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 },
  ...
  videoMoreMenuText: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 2, textTransform: 'uppercase', letterSpacing: 0.8, flexShrink: 1 },
  videoMoreMenuDivider: { height: 1, marginVertical: 4 },
  copyBlock: { borderRadius: 12, padding: 12, gap: 6 }, body: { ...fontSize.b5, lineHeight: 20 }, commentsBlock: { gap: 12, borderTopWidth: 1, paddingTop: 14 }, commentsPreview: { backgroundColor: '#202020', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, gap: 12 }, commentsPreviewHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, commentsPreviewTitle: { color: '#fff', ...fontSize.b4, lineHeight: fontSize.b4.fontSize + 2 }, commentsPreviewCount: { color: '#aaa' }, sectionTitle: { ...fontSize.b3, lineHeight: fontSize.b3.fontSize + 2 }, commentComposer: { borderRadius: 20, borderWidth: 1, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 4 }, commentInput: { flex: 1, minHeight: 40, ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, commentRow: { flexDirection: 'row', alignItems: 'center', gap: 10 }, commentAvatar: { width: 34, height: 34, borderRadius: 17 }, commentInitial: { backgroundColor: '#42a847', alignItems: 'center', justifyContent: 'center' }, commentInitialText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, commentUser: { ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, commentBody: { color: '#fff', ...fontSize.b5, lineHeight: 18 },
  queueHeaderRow: { flexDirection: 'row', alignItems: 'center', gap: 10 }, queueBadge: { borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 }, queueBadgeText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, manageButton: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 18, paddingHorizontal: 12, paddingVertical: 9 }, upNextLoader: { minHeight: 96, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center' }, upNextSkeletonStack: { gap: 12 }, upNextRow: { flexDirection: 'row', gap: 10 }, thumbWrap: { width: 156, aspectRatio: 16 / 9, borderRadius: 8, overflow: 'hidden', backgroundColor: '#000' }, thumb: { width: '100%', height: '100%' }, nextBadge: { position: 'absolute', top: 6, left: 6, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3 }, nextBadgeText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, durationBadge: { position: 'absolute', right: 6, bottom: 6, backgroundColor: 'rgba(0,0,0,0.82)', borderRadius: 4, paddingHorizontal: 5, paddingVertical: 2 }, durationText: { color: '#fff', ...fontSize.b5, lineHeight: fontSize.b5.fontSize + 1 }, upNextCopy: { flex: 1, justifyContent: 'space-between', paddingVertical: 1 }, upNextTitle: { ...fontSize.b5, lineHeight: 18 }, queueButton: { height: 30, borderRadius: 15, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingHorizontal: 10, alignSelf: 'flex-start' },
